#!/bin/bash
# M23b: Regression mode — predict continuous drug response (negated LMFI) + scGPT embeds
# Same M14e architecture but with MAE loss instead of BCE.
# Hypothesis: continuous targets provide smoother gradients, less noise than + more stable training w scGPT
...
conda run -n hgb_env --no-capture-output python scripts/train.py \
    ...
    --model_name M23b_regression_scgpt \
