...
class FeatureLoader:
    ...
    """
    def __init__(self, dataset, device, embedding_dir="data/embeddings", cell_feature_source="vae"):
        ...
        if cell_feature_source == "hybrid":
            vae = self._load_cell_features()
            multiomic = self._load_multiomic_cell_features()
            self.cell_features = torch.cat([vae, multiomic], dim=1).to(self.device)
            print(f"  > Hybrid cell features: VAE({vae.shape[1]}) + multiomic({multiomic.shape[1]}) = {self.cell_features.shape[1]}")
        elif cell_feature_source == "multiomic":
            self.cell_features = self._load_multiomic_cell_features().to(self.device)
        elif cell_feature_source == "scgpt":
            self.cell_features = self._load_scgpt_cell_features().to(self.device)
        else:
    ...
    def _load_drug_features(self):
        ...
        path = os.path.join(self.embedding_dir, "drugs_with_embeddings.csv")
    ...
    def _load_gene_features(self):
        ...
        # Adjust path relative to project root
        path = os.path.join(self.embedding_dir, "gene_embeddings_esm_by_symbol.pkl")
        if not os.path.exists(path):
    ...
    def _load_cell_features(self):
        ...
        # --- Define Paths ---
        embed_path = os.path.join(self.embedding_dir, "final_vae_cell_embeddings.npy")
        names_path = os.path.join(self.embedding_dir, "final_vae_cell_names.txt")
    ...
    def _load_scgpt_cell_features(self):
        """Load scGPT foundation model cell embeddings (512-dim).

        Uses same name-based alignment as VAE features.
        Files: scgpt_cell_embeddings.npy, scgpt_cell_names.txt
        """
        cell_type_id = self.dataset.node_name2type.get("cell", -1)
        if cell_type_id == -1 or cell_type_id not in self.dataset.nodes['count']:
            print("  > Warning: Cell type not found or has no nodes. Returning empty tensor.")
        ...
        num_cells = self.dataset.nodes['count'][cell_type_id]
        print(f"  > Aligning scGPT features for {num_cells} 'cell' nodes found in graph.")

        embed_path = os.path.join(self.embedding_dir, "scgpt_cell_embeddings.npy")
        names_path = os.path.join(self.embedding_dir, "scgpt_cell_names.txt")

        # Fall back to data_dir/embeddings if not in embedding_dir
        if not os.path.exists(embed_path):
            alt_dir = os.path.join(self.dataset.data_dir, "embeddings")
            alt_path = os.path.join(alt_dir, "scgpt_cell_embeddings.npy")
            if os.path.exists(alt_path):
                embed_path = alt_path
                names_path = os.path.join(alt_dir, "scgpt_cell_names.txt")

        if not os.path.exists(embed_path):
            print(f"  > FATAL ERROR: scGPT embedding file not found at {embed_path}")
            print(f"  > Run: python scripts/extract_scgpt_embeddings.py --data_dir {self.dataset.data_dir}")
            sys.exit(1)
        ...
        if len(names_data) != embeds_data.shape[0]:
            print(f"  > FATAL ERROR: Mismatch between names ({len(names_data)}) and embeddings ({embeds_data.shape[0]})")
            sys.exit(1)

        feature_dim = embeds_data.shape[1]
        cell_name_to_feat = {name: embeds_data[i] for i, name in enumerate(names_data)}
        print(f"  > Loaded {len(cell_name_to_feat)} scGPT cell features (Dim: {feature_dim}).")
        ...
        print(f"  > Aligned scGPT cell features: {aligned_cell_feats.shape}. Loaded {loaded_count}/{num_cells}.")
        if loaded_count != num_cells:
            print(f"  > WARNING: {num_cells - loaded_count} cells have zero embeddings (no expression data).")
        return aligned_cell_feats

    def _load_multiomic_cell_features(self):
        """Load pre-built multi-omic cell features (from build_cell_features.py).

        Features are already aligned by local cell ID.
        Shape: (N_cells, 964*4 + 2) = (N_cells, 3858)
        """
        # Look in dataset's data_dir first, fall back to data/processed
        path = os.path.join(self.dataset.data_dir, "cell_features_multiomic.pt")
        if not os.path.exists(path):
            path = "data/processed/cell_features_multiomic.pt"
        if not os.path.exists(path):
            print(f"  > FATAL ERROR: Multi-omic cell features not found")
            print(f"  > Run: python scripts/build_cell_features.py --data_dir {self.dataset.data_dir}")
            sys.exit(1)

        tensor = torch.load(path, weights_only=True)
        print(f"  > Loaded multi-omic cell features: {tensor.shape} (Dim: {tensor.shape[1]})")
        return tensor