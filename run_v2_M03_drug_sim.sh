#!/bin/bash
# V2 M03: Drug-Drug similarity edges (Morgan2 Tanimoto KNN)
# Same as M01 but with Drug-Drug similarity edges (edge type 5) in the graph.
# New drugs can now borrow response profiles from structurally similar known drugs.
# Architecture: M25 (dual head + dynamic isolation + Cell-Drug in GNN)
# Data: processed_v2 (ratio-matched GMM labels, 77% Sanger concordance)
# Evaluates on: DepMap test sets + all 4 Sanger scenarios
...
for scenario in ['S1', 'S2', 'S3', 'S4']:
    ...
    with torch.no_grad():
        for s_lids, t_lids, labels, s_types in loader:
            ...
            # Use inductive head for S2, S3, S4 (new cells or new drugs)
            # Use transductive head for S1 (known cells + known drugs)
            use_ind = scenario != 'S1'
...
conda run -n hgb_env --no-capture-output python scripts/plot_diagnostics_v2.py \
    --model_name v2_M03_drug_sim \
    --data_dir data/processed_v2 \
    --gpu 0

echo "All plots saved to results/v2_M03_drug_sim/"
