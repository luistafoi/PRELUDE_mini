#!/bin/bash
# V2 M05: Same as M04 but with PROPER WEIGHTED MEAN aggregation for drug-drug edges.
# Current code already multiplies neighbors by Tanimoto weight at gather time
# (a "biased mean": numerator is weighted but denominator is count-of-neighbors).
# M05 replaces the denominator with sum-of-weights for drug-drug pairs only,
# giving a true weighted mean. All other edge types keep the count-based behavior.
# Data: data/processed_v2_dedup/ (same as M04).

set -e

MODEL_NAME="v2_M05_weighted_dd"
DATA_DIR="data/processed_v2_dedup"
...
generator = DataGenerator('$DATA_DIR', include_cell_drug=True, dedup_symmetric=True)
...
args = argparse.Namespace(
    ...
    backbone_lr_scale=1.0, weighted_agg_pairs='drug:drug',
)
...
for scenario in ['S1', 'S2', 'S3', 'S4']:
    ...
    if len(np.unique(binary)) == 2:
        ...
        print(f'  {scenario}: Only one class ({len(all_labels):,} pairs)')
print()
print('Done.')
"
...
echo ""
echo "=========================================="
echo "COMPLETE — compare to M04 (dedup) baseline:"
echo "  M04: Test Ind=0.956  Trans=0.957  S1=0.848  S2=0.574  S3=0.842  S4=0.561"
echo "=========================================="
